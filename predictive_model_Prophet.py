...
class DateFeatureGenerator:
    ...
    def get_promotional_dates(self, date: Union[str, datetime]) -> List[datetime]:
        ...
        for year in range(2022, end_year + 1):
            thanksgiving = self.get_nth_weekday(year, 11, 4, 3)
            dates.extend([
    ...
    def get_weekday(date: Union[str, datetime]) -> int:
        ...
        return weekday_mapping[date.day_name()]
...
class ProphetForecaster:
    ...
    def prepare_prophet_data(self, df: pd.DataFrame, target_col: str) -> pd.DataFrame:
        ...
        return df.rename(columns={'date': 'ds', target_col: 'y'})
...
def main():
    ...
    spd_mape, _ = spd_forecaster.train_and_evaluate(spd_prophet_data)
    ...
    sales_mape, _ = sales_forecaster.train_and_evaluate(sales_prophet_data)
    sales_forecast = sales_forecaster.forecast(sales_prophet_data)
    
    # Print results
    print(f'Spend Forecast MAPE: {spd_mape:.2%}')
    print(f'Sales Forecast MAPE: {sales_mape:.2%}')
    
    return spd_forecast, sales_forecast

if __name__ == "__main__":
    spd_forecast, sales_forecast = main()
